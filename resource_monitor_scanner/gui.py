...
def launch_gui(csv_name='system_usage_log.csv'):
    while True:
        ...
        if chart_type not in ['Violin Plot']:
            ...
            if columns:
                if chart_type == 'Line Chart':
                    ...
                    plt.pie(average_values, labels=average_values.index,
                            autopct='%1.1f%%')
        ...
        rows_count = int(len(data) * 0.1)

        # Select the last 10% of the DataFrame
        last_10_percent = data.tail(rows_count)

        # Calculate the average of the last 10%
        average_last_10_percent = last_10_percent.mean()
        statistics = {
            'Minimum': data.min(),
            'Maximum': data.max(),
            'Average': data.mean(),
            'Average ( last 10%)': average_last_10_percent
        }
